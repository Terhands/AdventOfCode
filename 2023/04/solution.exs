...
# As you exit the gondola, the first thing you notice is that the air here is much warmer than it was on
# Snow Island. It's also quite humid. Is this where the water source is?
...
# "I'm not sure; I just operate the gondola lift. That does sound like something we'd have, though - this
# is Island Island, after all! I bet the gardener would know. He's on a different island, though - er, the
...
# numbers! That means card 1 is worth 8 points (1 for the first match, then doubled three times for each of
...
# Just as you're about to report your findings to the Elf, one of you realizes that the rules have actually been printed on the back of every
# card this whole time.

# There's no such thing as "points". Instead, scratchcards only cause you to win more scratchcards equal to the number of winning numbers you
# have.
...
# Process all of the original and copied scratchcards until no more scratchcards are won. Including the original set of scratchcards, how many
...
defmodule Day4 do
  ...
  defp to_card(card_data), do: (
    [card_name, rest] = card_data |> String.split(":", trim: true)
    ["Card", card_id]  = String.split(card_name, " ", trim: true)
    [winning_numbers_str, actual_numbers_str] = rest |> String.split("|", trim: true)
    {
      card_id,
      winning_numbers_str |> parse_numbers(),
      actual_numbers_str |> parse_numbers()
    }
  )

  defp total_matches({card_id, winning_numbers, actual_numbers}), do: (
    length = actual_numbers
  ...
  defp card_score({card_id, winning_numbers, actual_numbers}), do: (
    {_, value} = total_matches({card_id, winning_numbers, actual_numbers})
    cond do
      value == 0 -> value
      true -> trunc(:math.pow(2, value - 1))
    end
  )
  ...
  defp card_score_2([{card_id, 0} | rest]), do: (
    [card_id]
  )
  defp card_score_2([{card_id, total_winners} | []]), do: (
    [card_id]
  )
  defp card_score_2([{card_id, total_winners} | rest]), do: (
    cards = Enum.map(1..total_winners, fn (offset) ->
      card_score_2([
        Enum.at(rest, offset-1) |
        Enum.slice(rest, offset, length(rest))
      ])
    end) |> List.flatten()
    [card_id] ++ cards
  )
  ...
  defp part2(cards), do: (
    ...
    matches = cards
    |> Enum.map(fn(card) -> total_matches(card) end)
    Enum.map(0..length(matches)-1, fn(index) ->
      length(card_score_2([Enum.at(matches, index) | Enum.slice(matches, index+1, length(matches))]))
    end) |> Enum.sum
  )
end
