...
# to explain the situation, but it's a bit of a walk, so you have some time. They don't get many visitors up here;
...
# cubes and 4 red cubes; the second set is 1 red cube, 2 green cubes, and 6 blue cubes; the third set is only 2
# green cubes.
...
is_possible = fn (input) ->
    case String.split(input, " ", trim: true) do
      [total, "blue"] -> String.to_integer(total) <= max_blue
      [total, "green"] -> String.to_integer(total) <= max_green
      [total, "red"] -> String.to_integer(total) <= max_red
    end
end

game_value = fn (game_input_str) ->
  ["Game " <> game_id, game_input] = String.replace(game_input_str, ";", ",") |> String.split(": ")
  result = String.replace(game_input, ";", ",")
  |> String.split(",")
  |> Enum.map(is_possible)
  |> Enum.all?()
...
# minimum set of cubes in game 1 is 48. In games 2-5 it was 12, 1560, 630, and 36, respectively. Adding up these five powers
...
max_value = fn (game_input, colour) ->
  String.replace(game_input, ";", ",")
  |> String.split(",")
  |> Enum.map(&String.split(&1, " ", trim: true))
  |> Enum.filter(&match?([_, ^colour], &1))
  |> Enum.map(fn([total, _]) -> String.to_integer(total) end)
  |> Enum.max()
end

game_power = fn (game_input_str) ->
  ["Game " <> _, game_input] = String.replace(game_input_str, ";", ",") |> String.split(":")
  Enum.product([max_value.(game_input, "red"), max_value.(game_input, "blue"), max_value.(game_input, "green")])
end
